...
class Checkout extends Member_Controller
{
		...
		if ($submit == 'submit') {
	        ...
	        if ($this->form_validation->run() == FALSE)
	        {
				...
				*/
				...
				$this->db->where('user_id', $insert['customer_id']);
	...
	private function _repeat_view($user_id)
	{
		...
		$day_day = date('l', strtotime($data['hidden_date']['0']));

		$data['delivery_settings'] = $this->settings->delivery_session_settings($day_day);
	...
	public function payment_option_check($option)
	{
		...
		if ($option == 'transfer') {
			$data['total_expenses'] = sprintf('%0.2f',count_total_amount($customer_id)+deliveryfee()); // added delivery fee 
			if ($data['total_expenses'] < $minimum_spending) {
				$this->form_validation->set_message('payment_option_check', "Spend at least RM $minimum_spending and above.");
                return FALSE;				
			}else{
				return TRUE;
			}
