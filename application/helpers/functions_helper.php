...
if (! function_exists('get_food_type')) {
	function get_food_type($id)
	{
		...
		$ci->db->select('id,type,menu_english,menu_chinese');
...
if ( ! function_exists('json_to_array'))
{
    function json_to_array($picked_menu)
    {
		...
		for ($i=0; $i < count($picked_menu); $i++) {
				...
				if (isset($picked_menu[$i]['id'])) {
					$ci->db->where('id', $picked_menu[$i]['id']);
				}else{
					$ci->db->where('id', $picked_menu[$i]);
				}
