...
class Login extends MX_Controller
{
	...
	public function fb_connect()
	{
		...
		$connect_profile = $this->connect_to_facebook($fb_data);
	}


	private function connect_to_facebook($data)
	{

		$customer_id = $this->session->userdata('customer_id');
		// check if this account has connected to facebook 
		...
		if ($query->num_rows() == 0) {
			//if no, update to correspond customer id
			$update_data = array(
				'fb_id'    => $data['fb_id'],
				'fb_email' => $data['email'],
				'gender'   => $data['gender'],
			...
			$this->db->set('fb_email', $data['email']);
			$this->db->set('gender', $data['gender']);
			$this->db->set('fb_verified', $data['verified']);
			$this->db->set('is_email_active','1');
			...
			$this->session->set_flashdata('msg', 'This facebook account has been connected to other account.');
			redirect('account/profile'); 
		}
	}
