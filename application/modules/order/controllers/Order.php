...
class Order extends Member_Controller {
	...
	function order_ajax_add()
	{
		...
		if ($msg['condition'] == 'success') {
			...
			$cart_detail = array(
				...
				'expire_date'   => date('Y-m-d',(strtotime ( '-1 day', strtotime($user_input['that_day_date']))))." 22:00:00",
				);
	...
	private function _box_validation($food_code)
	{
		...
		if (!isset($box_price)) {
			...
			$msg['option_string'] = "Please select <strong>(1 vege 1 meat)</strong> <strong>(1 vege 2 meat)</strong> <strong>(2 vege 1 meat)</strong> <strong>(2 meat)</strong> <strong>(3 vege)</strong>";
			return $msg;
		}else{
