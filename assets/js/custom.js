$( document ).ready(function() {
	...
	$('#mealbox_modal').on('hidden.bs.modal', function (e) {	  //reset form
	...
	$('#alacarte_modal').on('shown.bs.modal', function (e) {
		var id = $(e.relatedTarget).data('id');
		$('#george').html('loading...').load('http://localhost.com/lovelymealbox/meal/ajax_alacarte?id='+id);

	});

	var alcarte_form = $('#alacarte-form');	
		alcarte_form.on('change',function(e){
		var alacarte_price = parseFloat($("#alacarte-price").text()).toFixed(2);		
    	var qty1 = parseInt($("#alacarte-qty").val());
    	var total = parseFloat(alacarte_price*qty1).toFixed(2);
    	$("#alacarte-total").text(total); 
	});

	$('#alacarte_submit').click(function(e){
		var url = "http://localhost.com/lovelymealbox/meal/ajax_add_alacarte";
	      e.preventDefault();
		    $.ajax({
		        url : url,
		        type: "POST",
		        dataType: "JSON",
		        data: $('#alacarte-form').serializeArray(),
		        success: function(data)
		        { 
		           if (data.status == 'failed') {
		           	 $('#alacarte_message').html("<div class='alert alert-danger alert-dismissable fade in'><a href='#'' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Unknown Error</div>");
		           }else{
		           	  location.reload();
		           }
		        },
	...
	$('.del_item').click(function(e){
		var url = "http://localhost.com/lovelymealbox/meal/delete_item"
		e.preventDefault();
	...
	$('#mealbox_submit').click(function(e){
		var url = "http://localhost.com/lovelymealbox/meal/ajax_meal_add";
	      e.preventDefault();
		    $.ajax({
		        url : url,
		        type: "POST",
	...
	quantity_box.on('change',function(e){
		...
		$.ajax({
			url: 'http://localhost.com/lovelymealbox/cart/update',
			type: 'post',
...
var cart = {
	'remove' :function(key) {
		$.ajax({
			url: 'http://localhost.com/lovelymealbox/cart/remove',
			type: 'post',
			data: 'key=' + key,
			dataType: 'text',
			success: function(json) {
